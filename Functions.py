...
def strategy_evaluate(equity, select_stock):
    ...
    year_return = equity[['涨跌幅', '指数涨跌幅']].resample(rule='A').apply(lambda x: (1 + x).prod() - 1)
    monthly_return = equity[['涨跌幅', '指数涨跌幅']].resample(rule='M').apply(lambda x: (1 + x).prod() - 1)

    year_return['超额收益'] = year_return['涨跌幅'] - year_return['指数涨跌幅']
    monthly_return['超额收益'] = monthly_return['涨跌幅'] - monthly_return['指数涨跌幅']

    year_return['涨跌幅'] = year_return['涨跌幅'].apply(num_to_pct)
    year_return['指数涨跌幅'] = year_return['指数涨跌幅'].apply(num_to_pct)
    year_return['超额收益'] = year_return['超额收益'].apply(num_to_pct)

    monthly_return['涨跌幅'] = monthly_return['涨跌幅'].apply(num_to_pct)
    monthly_return['指数涨跌幅'] = monthly_return['指数涨跌幅'].apply(num_to_pct)
